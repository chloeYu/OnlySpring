				...
				if (positionOfhash == null || positionOfspace == null)
					...
					for (var i = 0; i < positionOfhash.length; i++) {
						if (positionOfspace[i] == null) {
							if(text.indexOf("#",positionOfhash[i]+1)>-1){
								console.log(text.slice(positionOfhash[i], text.indexOf("#",positionOfhash[i]+1)));
								hashtag.push(text.slice(positionOfhash[i], text.indexOf("#",positionOfhash[i]+1)));
							} else {
								console.log("hashTag not finished");
								console.log(text.slice(positionOfhash[i]));
								hashtag.push(text.slice(positionOfhash[i]));						
							}
						} else {
							if(text.indexOf("#",positionOfhash[i]+1)>-1 && text.indexOf("#",positionOfhash[i]+1)<positionOfspace[i]){
								console.log(text.slice(positionOfhash[i], text.indexOf("#",positionOfhash[i]+1)));
								hashtag.push(text.slice(positionOfhash[i], text.indexOf("#",positionOfhash[i]+1)));
							}
							else{
					...
					for (var i = 0; i < hashtag.length; i++) {
						if(hashtag[i]=="#")
							...
							$("#hashtag").val($("#hashtag").val() + hashtag[i]);
					}
				}
			});
	$(".friend_list").on(
			...
			function() {
				if (a) {
					$(".friendlistplace").addClass("disapper").removeClass(
							"show");
				} else {
					$.get("/only/friendList", "member_id="
							+ $("#member_id").val(), function(data) {
						$("#friendload").html(data);
					});
					$(".friendlistplace").removeClass("disapper").addClass(
							"show");
				}
				a = !a;
			});
...
$(function() {
	$('.dimClose').on(
			...
			function() {
				...
				$(this).parents().children('.det').addClass('hide')
						.removeClass('show');
				// }
			});
});
...
function friendupdate(status, uid1, uid2) {
	$.post("/only/friendupdate/" + status + "/" + uid1 + "/" + uid2, function(
			data) {
		$.get("/only/friendList", "member_id=" + $("#member_id").val(),
				function(data) {
					$("#friendload").html(data);
				});
	});
